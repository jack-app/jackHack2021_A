...
async function all() {
  let resp = await fetch(`https://42f1c748675f.ngrok.io/camp_encamped_locations`, {mode: 'cors'})
  return resp.json()
}
...
function save(camp_encamped_location) {
  ...
  fetch( 'https://42f1c748675f.ngrok.io/camp_encamped_locations', { method: "POST", body: JSON.stringify(camp_encamped_location), mode: "cors" } )
...
function deleteCEL(id) {
  ...
  fetch( `https://42f1c748675f.ngrok.io/camp_encamped_locations/${id}`, { method: "DELETE", mode: "cors" } )
  .then(resp => console.log(resp))
}